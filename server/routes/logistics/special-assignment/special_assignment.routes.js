  ...
  router.post("/", async (req, res) => {
    try {
      ...
      pool.query(query, values, (err, result) => {
        if (err) throw err;
        res.status(201).json({ message: "Special assignment created successfully." });
      });
    } catch (error) {
    ...
    try {
      ...
      pool.query(query, values, (err, result) => {
        ...
        if (result.affectedRows > 0) {
          res.status(200).json({ message: "Special assignment updated successfully." });
        } else {
          res.status(404).json({ message: "Special assignment not found." });
        }
      });
...
router.delete("/:id",  async (req, res) => {
  try {
    ...
    pool.query(query, [id], (err, result) => {
      if (err) throw err;
      if (result.affectedRows > 0) {
        res.status(200).json({ message: "Special assignment deleted successfully." });
      } else {
        res.status(404).json({ message: "Special assignment not found." });
      }
    });
