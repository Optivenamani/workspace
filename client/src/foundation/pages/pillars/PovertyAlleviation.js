...
const PovertyAlleviation = () => {
  ...
  const filteredPoverty = useMemo(() => {
    return poverty.filter((item) => {
      if (searchQuery === "") {
        return true; // Include all items when the search query is empty
      } else if (
        item.poverty_name &&
        item.poverty_name.toLowerCase().includes(searchQuery.toLowerCase())
      ) {
                    ...
                    <div className="overflow-hidden border border-gray-200 dark:border-gray-700 md:rounded-lg">
                      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-900">
                          {filteredPoverty.map((poverty, index) => (
                            <tr key={index}>
